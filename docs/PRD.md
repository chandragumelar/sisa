...
| 4.4  | Sebagai user, gue mau melihat budget minggu ini dan rasio beban tagihan dalam 2 kolom        | Grid 2 kolom; kolom kiri: budget minggu ini; kolom kanan: "TAGIHAN VS UANGMU" — `unpaidTagihanTotal ÷ totalSaldo × 100%`; verdict: <30% "masih sehat" (signal-safe), 30–60% "lumayan" (signal-caution), >60% "berat" (signal-danger); saldo 0 → tampil "—"; urutan JSX: hero → bar → grid |
